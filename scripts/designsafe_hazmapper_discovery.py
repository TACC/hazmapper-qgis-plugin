...
def generate_config_file(projects_with_maps: List[Dict], output_file: str = "maps_of_published_projects.py"):
    """
...
def generate_readme(projects_with_maps: List[Dict], output_file: str = "README_PUBLISHED_MAPS.md"):
    """
...
'''
    ...
    for project in projects_with_maps:
        project_id = project['projectId']
        project_title = project['title'].replace('|', '\\|')  # Escape pipes for markdown
        
        for hazmapper_map in project['hazmapperMaps']:
            uuid = hazmapper_map.get('uuid', 'N/A')
            hazmapper_id = hazmapper_map.get('hazmapper_project_id', 'N/A')
            currently_working = '✅' if hazmapper_map.get('currently_working', False) else '❌'
            public = '✅' if hazmapper_map.get('public', False) else '❌'
            
            hazmapper_url = f"https://hazmapper.tacc.utexas.edu/hazmapper/project-public/{uuid}/"
...
def main():
            ...
            if hazmapper_maps:
                ...
                for hm_map in hazmapper_maps:
                    ...
                    if uuid:
                        ...
                        status_msg = "✅ Working" if hazmapper_status['currently_working'] else "❌ Not accessible"
                        public_msg = "Public" if hazmapper_status['public'] else "Private"
                        print(f"      {status_msg}, {public_msg}, ID: {hazmapper_status.get('hazmapper_project_id', 'N/A')}")
                    else:
    ...
    for project in projects_with_maps:
        ...
        for hm_map in project['hazmapperMaps']:
            status = "Working" if hm_map.get('currently_working', False) else "Not working"
            public = "Public" if hm_map.get('public', False) else "Private"
            print(f"    UUID: {hm_map.get('uuid', 'N/A')}, ID: {hm_map.get('hazmapper_project_id', 'N/A')}, Status: {status}, {public}")
